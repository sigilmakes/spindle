...
import { summarizeRun } from "./manager.js";

export interface SpindleThreadDetails {
    run: ThreadRun;
}
...
export function formatThreadRun(run: ThreadRun, theme: Theme, expanded: boolean = false): string {
    const lines: string[] = [];
    ...
    lines.push(`  ${theme.fg("accent", summarizeRun(run))}`);
    ...
    if (run.phases.length > 0) {
        ...
        for (const phase of run.phases) {
            const done = phase.agents.filter((a) => a.status === "done" || a.status === "cached").length;
...
export function renderThreadResult(
    result: AgentToolResult<SpindleThreadDetails>,
    expanded: boolean,
    theme: Theme,
): Text {
    ...
    return new Text(formatThreadRun(details.run, theme, expanded), 0, 0);
}

export function formatThreadList(runs: ThreadRun[], theme: Theme): string {
    if (runs.length === 0) return theme.fg("muted", "No threads have run yet.");
    return runs.map((run) => {
        const head = `${colorStatus(run.status, theme, icon(run.status))} ${theme.fg("toolTitle", run.name)} ${theme.fg("dim", run.id)}`;
        return `${head}\n  ${theme.fg("muted", summarizeRun(run))}`;
    }).join("\n\n");
}
