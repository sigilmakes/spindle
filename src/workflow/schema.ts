...
function validateValue(value: unknown, schema: JsonSchema, path: string, errors: string[]): void {
    ...
    if (schema.type === "array" && Array.isArray(value) && schema.items) {
        value.forEach((item: unknown, index: number) => validateValue(item, schema.items!, path + "[" + index + "]", errors));
    }
}
...
export function extractJson(text: string): unknown {
    ...
    if (firstObj >= 0 && lastObj > firstObj) return JSON.parse(trimmed.slice(firstObj, lastObj + 1));
    const firstArr = trimmed.indexOf("[");
    const lastArr = trimmed.lastIndexOf("]");
    if (firstArr >= 0 && lastArr > firstArr) return JSON.parse(trimmed.slice(firstArr, lastArr + 1));
