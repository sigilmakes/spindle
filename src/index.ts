    ...
    pi.registerTool({
        ...
        promptGuidelines: [
            [
                ...
                "",
                "  // Compose — search then fetch:",
                "  hits = await mcp_call('searxng', 'search', { query: 'react server components' })",
                "  urls = hits.output.match(/https?:\\/\\/\\S+/g).slice(0, 3)",
                "  pages = await Promise.all(urls.map(u => mcp_call('markit', 'fetch', { url: u })))",
                "",
                "  // Persistent proxy (for repeated calls to same server):",
