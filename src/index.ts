...
export default function spindle(pi: ExtensionAPI) {
    ...
    pi.registerTool({
        ...
        promptGuidelines: [
            ...
            "**Guard your context window.** Prefer `load()` over `read()` — load stores data in a variable without outputting it. Use `grep`, `bash` with `awk`/`jq`/`head`/`tail` to extract what you need. Only `console.log` the specific lines or values you need to see — never dump entire files.",
